        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="mb-8">
                        <h3 class="text-lg font-semibold mb-4">{{ $review->requestModel->title }}</h3>
                        <p class="text-gray-600 mb-4">{{ $review->requestModel->description }}</p>
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div>
                                <p class="text-sm text-gray-500">Fecha de Completado</p>
                                <p class="font-medium">{{ $review->requestModel->completed_at?->format('d/m/Y') ?? 'No especificada' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            <div class="border rounded-lg p-4 mb-4">
                                <div class="flex justify-between items-center mb-4">
                                    <div>
                                        <h5 class="font-medium">{{ $review->assistant->name }}</h5>
                                    </div>
                                </div>

                                <form action="{{ route('needhelp.reviews.update', $review) }}" method="POST" class="space-y-4">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <div class="mt-2 flex items-center space-x-4">
                                            @for($i = 1; $i <= 5; $i++)
                                                <label class="relative">
                                                    ...
                                                    <svg class="w-8 h-8 cursor-pointer {{ $i <= $review->rating ? 'text-yellow-400' : 'text-gray-300' }} hover:text-yellow-400 transition-colors" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="comment" id="comment" rows="3" 
                                            ...
                                            placeholder="Cuéntanos tu experiencia con este asistente...">{{ old('comment', $review->comment) }}</textarea>
                                        @error('comment')
                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="flex justify-end space-x-4">
                                        <a href="{{ route('needhelp.requests.show', $review->requestModel) }}" 
