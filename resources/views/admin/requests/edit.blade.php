                    ...
                    <form action="{{ route('admin.requests.update', $requestModel) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                <option value="open" {{ old('status', $requestModel->status) == 'open' ? 'selected' : '' }}>Abierta</option>
                        ...
                        <div>
                            ...
                            <x-text-input id="location" name="location" type="text" class="mt-1 block w-full" :value="old('location', $requestModel->location)" />
                            <x-input-error class="mt-2" :messages="$errors->get('location')" />
                        </div>

                        <div>
                            <x-input-label for="deadline" :value="__('Fecha límite')" />
                            <x-text-input id="deadline" name="deadline" type="datetime-local" class="mt-1 block w-full" :value="old('deadline', $requestModel->deadline ? $requestModel->deadline->format('Y-m-d\TH:i') : '')" />
